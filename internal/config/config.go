...
func (c *Config) Validate() error {
	...
	if c.Camera.Rotation != 0 {
		switch c.Camera.Mode {
		case "v4l2", "rpicamvid":
			// v4l2 transposes in-process; rpicamvid switches the
			// subprocess to raw YUV420 and transposes in-process for
			// 90/270 (Pi libcamera itself rejects transpose transforms).
		default:
